...
def test_load_data_single_band(data_loader):
    ...
    mock_item = Item(
        id="test_item_1", geometry={}, bbox=[], datetime=datetime.now(), properties={}
    )
    ...
    with patch.object(data_loader, "download_asset") as mock_download:
        ...
        mock_download.assert_called_once_with(
            "http://example.com/B01.tif", saved_files[0]
        )
...
def test_load_data_multiple_bands(data_loader):
    ...
    mock_item = Item(
        id="test_item_2", geometry={}, bbox=[], datetime=datetime.now(), properties={}
    )
...
def test_load_data_no_assets(data_loader):
    ...
    mock_item = Item(
        id="test_item_3", geometry={}, bbox=[], datetime=datetime.now(), properties={}
    )
